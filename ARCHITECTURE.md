...
- `webview/WebViewExecutor` - **25 lines whose entire body is
  `WebSettings.getDefaultUserAgent(context)`.** It never constructs a
  `WebView`. (The Phase 0 draft said it runs a headless WebView to clear
  Cloudflare challenges. That was wrong and would have caused a
  headless-browser port to be budgeted for something that does not
  exist.) Cloudflare solving actually lives in `browser/cloudflare/` as a
  full Activity. The only `WebView` the app constructs anywhere is
  `lnreader/js/JsHost.kt`. `webview/adblock/` is a small rule engine.
...
The Phase 0 draft said the app consumes "only models and utils"; that is
refuted, it consumes 7 subpackages, and the one that matters is
`config`: `core/prefs/SourceSettings.kt:18` already declares
`class SourceSettings(context, source) : MangaSourceConfig`, so
`MangaLoaderContext.getConfig` is in effect **already implemented in this
repo**. The bulk of what it consumes is still `Manga`,
...
1. **`kotatsu-parsers` is a pure JVM jar.** Its Gradle module metadata
   ...
   `MangaLoaderContext` is an **8**-member abstract class
   (`getHttpClient`, `getCookieJar`, `evaluateJs(script)`,
   `evaluateJs(url, script)`, `getConfig`, `getDefaultUserAgent`,
   `redrawImageResponse`, `createBitmap`) and the library already
   abstracts bitmaps behind its own `parsers.bitmap.Bitmap` interface
   (`getWidth`/`getHeight`/`drawBitmap`, no pixel access) rather than
   `android.graphics`. `MangaParserSource` has exactly **1270**
   constants. Verified by running it: a stub context on a stock JDK 21
   instantiated all 1270 with zero failures and completed live
   `getList` -> `getDetails` -> `getPages` chains against real sites.
